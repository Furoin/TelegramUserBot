...
@Client.on_message(Filters.user(user_id) & Filters.command("purgeme", prefix))
def purgeme(client, message):
    if len(message.command) > 1:
        try:
            ...
            try:
                to_del = [message.id for message in client.send(
                    Search(peer=client.resolve_peer(message.chat.id), q="",
                           filter=InputMessagesFilterEmpty(),
                           min_date=0, max_date=0, offset_id=0, add_offset=0, limit=count,
                           max_id=0, min_id=0, hash=0,
                           from_id=client.resolve_peer(user_id)))["messages"]]
                i = len(to_del)
